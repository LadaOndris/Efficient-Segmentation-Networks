...
"""
BDD100K is dataset consisting of 100k images.
Lane labelling is available, there are 2 classes : ego lane and other lanes.
"""


class BDD100KDataSet(data.Dataset):
    """ 
       BDD100KDataSet is employed to load train set
       Args:
        root: the BDD100K dataset path, 
        list_path: bdd100k_train_list.txt, include partial path

    """

    def __init__(self, root='', list_path='', max_iters=None, crop_size=(176, 320),
                 mean=(128, 128, 128), scale=False, mirror=True, ignore_label=0):
        self.root = root
...
class BDD100KValDataSet(data.Dataset):
    """ 
       BDD100KValDataSet is employed to load val set
       Args:
        root: the BDD100K dataset path, 
        list_path: bdd100k_val_list.txt, include partial path
...
class BDD100KTestDataSet(data.Dataset):
    """ 
       BDD100KTestDataSet is employed to load test set
       Args:
        root: the BDD100K dataset path, 
        list_path: bdd100k_test_list.txt, include partial path
...
class BDD100KTrainInform:
    ...
    def __init__(self, data_dir='', classes=3, train_set_file="",
